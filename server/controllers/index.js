...
// Finds a dog by name and increments its age by 1
const findByNameAndUpdate = async (req, res) => {
  ...
  try {
    ...
    // Error in the find process
    console.log(err);
    return res.status(500).json({ error: 'Something went wrong' });
  }

  // Error for no dog found with given name
  if (!doc) {
  ...
  updatePromise.then((docfinal) => res.json({
    name: docfinal.name,
    breed: docfinal.breed,
    age: docfinal.age,
  }));
